CREATE OR REPLACE FUNCTION context.set(iid_session bigint, iis_local boolean DEFAULT true)
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
AS $function$
    ...
    then
      begin
        select u.Id_User,
         ...
         where s.Id_Session = iId_Session;
        

/*        execute 'set local timeZone = ' || quote_literal(vTZ);*/
        perform context.set('Id_Session', iId_Session::text, iIs_Local);
