CREATE OR REPLACE FUNCTION "cf$_import"."#homebuh5_2015_03_31"(iid_file integer, iimport_source_type_code integer, idelimiter text, iisgroup boolean, iid_tag integer, OUT oresult text)
 RETURNS text
 LANGUAGE plpgsql
 SECURITY DEFINER
AS $function$
    ...
    begin
      ...
      loop
        ...
        then
          ...
          then
	          perform error$.raise ('invalid_file_format', iMessage := 'Не найден столбец "Дата". Пожалуйста, проверте формат файла');
          end if;
          
          if vIndex_Account is null 
          then
	          perform error$.raise ('invalid_file_format', iMessage := 'Не найден столбец "Счет". Пожалуйста, проверте формат файла');
          end if;
          ...
          then
            ...
            then
  	          perform error$.raise ('invalid_file_format', iMessage := 'Не найден столбец "Категория расхода". Пожалуйста, проверте формат файла');
            else
  	          perform error$.raise ('invalid_file_format', iMessage := 'Не найден столбец "Категория дохода". Пожалуйста, проверте формат файла');
            end if;
          end if;
          ...
          then
            if iImport_Source_Type_Code = 1 
            then
  	          perform error$.raise ('invalid_file_format', iMessage := 'Не найден столбец "Подкатегория расхода". Пожалуйста, проверте формат файла');
            else
  	          perform error$.raise ('invalid_file_format', iMessage := 'Не найден столбец "Подкатегория дохода". Пожалуйста, проверте формат файла');
            end if;
          end if;

          if vIndex_Sub_Category is null 
          then
	          perform error$.raise ('invalid_file_format', iMessage := 'Не найден столбец "Кол.". Пожалуйста, проверте формат файла');
          end if;
          ...
          then
  	        perform error$.raise ('invalid_file_format', iMessage := 'Не найден столбец "Рубли|Сумма", "Евро|Сумма" или "Доллары|Сумма". Пожалуйста, проверте формат файла');
          end if;
    ...
    begin
      ...
      loop
        ...
        then
          ...
          then
	          perform error$.raise ('invalid_file_format', iMessage := 'Не найден столбец "Со счета". Пожалуйста, проверте формат файла');
          end if;
          if vIndex_Account_To is null 
          then
  	        perform error$.raise ('invalid_file_format', iMessage := 'Не найден столбец "На счет". Пожалуйста, проверте формат файла');
          end if;
          if vIndex_Sum is null 
          then
	          perform error$.raise ('invalid_file_format', iMessage := 'Не найден столбец "Сумма". Пожалуйста, проверте формат файла');
          end if;
        ...
        then
          ...
          then
            if iImport_Source_Type_Code = 4 then
  	          perform error$.raise ('invalid_file_format', iMessage := 'Не найден столбец "Должник". Пожалуйста, проверте формат файла');
            else
  	          perform error$.raise ('invalid_file_format', iMessage := 'Не найден столбец "Кредитор". Пожалуйста, проверте формат файла');
            end if;
          end if;
          if     vIndex_Euro is null 
             and vIndex_Dollar is null 
    ...
    begin
      ...
      loop
        ...
        then
          ...
          then
	          perform error$.raise ('invalid_file_format', iMessage := 'Не найден столбец "Отдано|Рубли", "Отдано|Евро" или "Отдано|Доллары". Пожалуйста, проверте формат файла');
          end if;
          ...
          then
  	        perform error$.raise ('invalid_file_format', iMessage := 'Не найден столбец "Получено|Рубли", "Получено|Евро" или "Получено|Доллары". Пожалуйста, проверте формат файла.');
          end if;
