CREATE OR REPLACE FUNCTION "core$_port".call_operation(ioperation_name text, iparams text, OUT oresponse text)
 RETURNS text
 LANGUAGE plpgsql
 SECURITY DEFINER
AS $function$
...
begin
  begin
    ...
    if vOperation.Is_Need_Authorize  then
      perform core$_auth.authorize (nullif (trim (vParams->>'authorization'), ''));
    end if;
